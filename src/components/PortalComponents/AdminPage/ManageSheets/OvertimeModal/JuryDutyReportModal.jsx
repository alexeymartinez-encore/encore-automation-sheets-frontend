...
export default function JuryDutyReportModal({ onClose }) {
  ...
  useEffect(() => {
    async function getTimesheets() {
      const res = await adminCtx.fetchJuryDutyData(selectedDate);
      setTimesheets(res || []);
    }
    getTimesheets();
  ...
  function downloadCSV() {
    ...
    link.href = url;
    link.download = `jury_duty_report_${
    ...
    >
      ...
      >
        <div className="flex justify-between items-center w-full">
          <h1 className="py-3 text-lg">Bi-Weekly Jury Duty Report</h1>
          <button
            onClick={downloadCSV}
